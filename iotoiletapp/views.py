...
select floor_name, count(room_type_name='男性用' or null) as male_empty, count(room_type_name='女性用' or null) as female_empty

from (

select * from iotoiletapp_sensordata as SD left outer join iotoiletapp_sensor as S
on S.id = SD.sensor_id_id
left outer join iotoiletapp_toilet as T
on S.toilet_id_id = T.id
...
on F.id = R.floor_id_id
where (sensor_id_id, timestamp) in (select sensor_id_id, max(timestamp) from iotoiletapp_sensordata
...
def index(request):
    with psycopg2.connect(CONN) as conn, conn.cursor() as cur:
        ...
        while available_toilets != None:
            if available_toilets[5] < 3:
                available_toilets[3] = "空"
            elif available_toilets[5] > 3:
                available_toilets[3] = "使用中"
            else:
                available_toilets[3] = "---"
